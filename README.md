...
- AWS + 백엔드로 백오피스 제작 중 (진행 중)
  - 리절트 화면 이외에도 기록과 관련된 이미지를 여러 개 볼 수 있었다면 좋겠다. (진행 중)
  - 기록 리스트에서 제목 이외의 정보도 알고 싶다. (진행 중)
- 모바일 화면에서 스크롤에 상관없이 헤더가 표시되었으면 좋겠다.
- 다크 모드를 지원했으면 좋겠다.
- 기록 외 갤러리나 게임 리뷰같은 컨텐츠도 있었으면 좋겠다.
